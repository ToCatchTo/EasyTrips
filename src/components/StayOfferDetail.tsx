...
const StayOfferDetail = () => {
    type StayOfferDetailData = {
        location: string;
    ...
    const [offer, setOffer] = useState<StayOfferDetailData | null>(null);
    ...
    useEffect(() => {
        const fetchStayOffer = async () => {
            if (!stayId) return;
            const offerData = await stayController.getStay(stayId);
            const reviewsData = await reviewController.getReviewsByOffer(stayId);
            setReviews(reviewsData);
            setOffer(offerData);
        };
        fetchStayOffer();
    }, [stayId, refreshTrigger]);
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box sx={{ display: 'flex', gap: '30px' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                            <Typography sx={{ fontSize: '36px', fontWeight: 'bold' }}>{offer?.location}</Typography>
                            <Box sx={{ display: 'grid', gridTemplateColumns: 'auto auto', gridTemplateRows: 'auto auto auto', justifyContent: 'space-between', height: '100%', columnGap: '30px' }}>
                                <Box sx={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                                    <Typography sx={{ fontWeight: 'bold' }}>Cena na noc za osobu:</Typography>
                                    <Typography>{offer?.pricePerNight} Kč</Typography>
                                </Box>
                                <Box sx={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                                ...
                                <Box sx={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                                    ...
                                    <Typography>{offer?.petsAllowed ? "Povoleni" : "Nepovoleni"}</Typography>
                                </Box>
                            </Box>
                        </Box>
                    </Box>
